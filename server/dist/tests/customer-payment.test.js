...
describe('POST /api/customers/:id/payments', () => {
    ...
    beforeEach(async () => {
        ...
        await test_util_1.CustomerTest.delete('NIM-');
    });
    afterEach(async () => {
        await test_util_1.CustomerTest.delete();
        await test_util_1.CustomerTest.delete('NIM-');
    ...
    it('should accept custom tanggalBayar (ISO string)', async () => {
        const c = await test_util_1.CustomerTest.create({ totalBayar: 50000, sudahBayar: 0, sisaBayar: 50000 });
        const res = await (0, supertest_1.default)(app_1.default)
            .post(`/api/customers/${c.id}/payments`)
            .set('Authorization', `Bearer ${ownerToken}`)
...
describe('PATCH /api/customers/:id/invoice', () => {
    ...
    it('should allow OWNER to update totalBayar & recompute sisaBayar from payments', async () => {
        const c = await test_util_1.CustomerTest.create({ totalBayar: 100000, sudahBayar: 0, sisaBayar: 100000 });
...
describe('GET /api/customers/:id/payments', () => {
    ...
    it('should return paginated list sorted by tanggalBayar desc by default', async () => {
        const c = await test_util_1.CustomerTest.create({ totalBayar: 999999, sudahBayar: 0, sisaBayar: 999999 });
        ...
        expect(items.map((x) => x.catatan)).toEqual(['C', 'B']); // default desc
    ...
    it('should filter by date range (start & end)', async () => {
        const c = await test_util_1.CustomerTest.create({ totalBayar: 100000 });
        await (0, supertest_1.default)(app_1.default).post(`/api/customers/${c.id}/payments`).set('Authorization', `Bearer ${ownerToken}`).send({ amount: 10000, tanggalBayar: '2025-01-10', catatan: 'JAN' });
        await (0, supertest_1.default)(app_1.default).post(`/api/customers/${c.id}/payments`).set('Authorization', `Bearer ${ownerToken}`).send({ amount: 20000, tanggalBayar: '2025-03-10', catatan: 'MAR' });
        await (0, supertest_1.default)(app_1.default).post(`/api/customers/${c.id}/payments`).set('Authorization', `Bearer ${ownerToken}`).send({ amount: 30000, tanggalBayar: '2025-07-10', catatan: 'JUL' });
        const res = await (0, supertest_1.default)(app_1.default)
            .get(`/api/customers/${c.id}/payments`)
            .query({ start: '2025-03-01', end: '2025-06-30', limit: 10 })
            .set('Authorization', `Bearer ${ownerToken}`);
        expect(res.status).toBe(200);
        expect(res.body.data.items.map((x) => x.catatan)).toEqual(['MAR']);
    });
    it('should return 404 when customer not found', async () => {
        const res = await (0, supertest_1.default)(app_1.default)
    ...
    it('should return 401 when no token provided', async () => {
        const c = await test_util_1.CustomerTest.create();
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/customers/${c.id}/payments`);
        expect(res.status).toBe(401);
        expect(res.body.status).toBe('error');
    });
