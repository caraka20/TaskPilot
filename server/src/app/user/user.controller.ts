...
export class UserController {
  static async register(req: Request, res: Response, next: NextFunction) {
    try {
      const request: RegisterRequest = await Validation.validate<RegisterRequest>(
  ...
  static async login(req: Request, res: Response, next: NextFunction) {
    try {
      const request: LoginRequest = await Validation.validate<LoginRequest>(
  ...
  static async getUserDetail(req: Request, res: Response, next: NextFunction) {
    try {
      const params = await Validation.validate(
      ...
      const tugas = Array.isArray((result as any)?.tugas)
        ? (result as any).tugas
  ...
  static async setJedaOtomatisUser(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const payload = await Validation.validate<SetJedaOtomatisRequest>(
        UserValidation.SET_JEDA_OTOMATIS,
        req.body
      );
      ...
      return ResponseHandler.success(
        res,
        result,
        SUCCESS_MESSAGES.UPDATED.JEDA_OTOMATIS
      );
    } catch (err) {
      next(err);
    }
  ...
  */
  static async getUserEverything(
    ...
    try {
      ...
      if (req.user?.role === Role.USER && req.user.username !== username) {
      ...
      const q = await Validation.validate(
        UserValidation.DETAIL_DEEP_QUERY,
        req.query as Record<string, any>
      );

      const data = await UserService.getUserEverything(username, q);
      return ResponseHandler.success(
        res,
        data,
        SUCCESS_MESSAGES.FETCHED.USER
