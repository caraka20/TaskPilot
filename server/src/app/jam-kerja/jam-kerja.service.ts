...
export class JamKerjaService {
  ...
  static async update(
    ...
    if (!Number.isFinite(nextJamMulai.getTime())) {
      throw AppError.fromCode(ERROR_CODE.BAD_REQUEST, "Jam mulai tidak valid");
    }
    if (nextJamSelesai && !Number.isFinite(nextJamSelesai.getTime())) {
      throw AppError.fromCode(ERROR_CODE.BAD_REQUEST, "Jam selesai tidak valid");
    }
    ...
    if (nextStatus === StatusKerja.SELESAI && !nextJamSelesai) {
      throw AppError.fromCode(
        ERROR_CODE.BAD_REQUEST,
        "Status SELESAI memerlukan jam selesai"
      );
    }
    if (nextStatus === StatusKerja.AKTIF && nextJamSelesai) {
      throw AppError.fromCode(
        ERROR_CODE.BAD_REQUEST,
        "Status AKTIF tidak boleh memiliki jam selesai"
      );
    }

    const previousTotalJam = Number(row.totalJam || 0);
    const previousLastSegment = row.jamSelesai
      ? JamKerjaRepository.calcTotalJam(row.jamMulai, row.jamSelesai)
      : 0;
    // Setelah resume, jamMulai menjadi baseline segmen terakhir sementara totalJam
    // tetap membawa durasi segmen sebelum jeda. Bagian carry ini tidak boleh hilang
    // ketika OWNER mengoreksi jam mulai/selesai sesi yang sudah selesai.
    const carriedJam = row.status === StatusKerja.SELESAI
      ? round2(Math.max(0, previousTotalJam - previousLastSegment))
      : previousTotalJam;

    let newTotalJam = previousTotalJam;
    if (nextStatus === StatusKerja.SELESAI) {
      if (row.status === StatusKerja.JEDA) {
        // Durasi saat JEDA sudah dibukukan ketika pause; waktu jeda bukan jam kerja.
        newTotalJam = previousTotalJam;
      } else {
        const correctedLastSegment = JamKerjaRepository.calcTotalJam(
          nextJamMulai,
          nextJamSelesai!
        );
        newTotalJam = round2(carriedJam + correctedLastSegment);
      }
    } else if (nextStatus === StatusKerja.AKTIF && row.status === StatusKerja.SELESAI) {
      // Saat sesi selesai dibuka kembali, segmen terakhir akan dihitung ulang ketika end.
      newTotalJam = carriedJam;
    }
    ...
    const deltaJam = payload.recalcGaji === false ? 0 : round2(newContrib - oldContrib);
    const rate = deltaJam === 0 ? 0 : await getRateFor(row.username);

    // JamKerja dan akumulasi User wajib berubah bersama. Jika salah satunya gagal,
    // Prisma me-rollback keduanya sehingga profil tidak menampilkan data setengah jadi.
    const updated = await prismaClient.$transaction(async (tx) => {
      if (nextIsOpen) {
        const otherOpenSession = await tx.jamKerja.findFirst({
          where: {
        ...
        if (otherOpenSession) {
          throw AppError.fromCode(
            ERROR_CODE.BAD_REQUEST,
            `Masih ada sesi aktif lain (#${otherOpenSession.id}) untuk user ini`
          );
        }
      }

      const saved = await tx.jamKerja.update({
        where: { id },
        data: {
      ...
      if (deltaJam !== 0) {
        await tx.user.update({
          ...
          data: {
            totalJamKerja: { increment: deltaJam },
            totalGaji: { increment: deltaJam * rate },
          },
        });
      }
