...
describe("GET /api/public/customers/:nim/tuton", () => {
  ...
  it("should return self-view with DISKUSI 1/2/4/6/8, TUGAS 3/5/7, and ABSEN 1-8", async () => {
    // NIM unik per test
    ...
    // Course 1 + default items (16 = 5 diskusi + 8 absen + 3 tugas)
    ...
    // Buat item di luar jadwal untuk memastikan endpoint publik memfilternya.
    await prismaClient.tutonItem.createMany({
      data: [
        { courseId: c1.id, jenis: JenisTugas.DISKUSI, sesi: 9, status: StatusTugas.BELUM },
        { courseId: c1.id, jenis: JenisTugas.ABSEN, sesi: 9, status: StatusTugas.BELUM },
        { courseId: c1.id, jenis: JenisTugas.TUGAS, sesi: 1, status: StatusTugas.BELUM },
      ],
    })
    ...
    expect(data.totalItems).toBe(32)
    expect(data.totalCompleted).toBe(4)
