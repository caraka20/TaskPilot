...
describe("GET /api/tuton-courses/:courseId/items", () => {
  ...
  it("should return 16 items with the UT session schedule (OWNER)", async () => {
    const customer = await CustomerTest.create()
    const course = await TutonTest.createCourse(customer.id, "MAN101 - Manajemen")
    await TutonTest.createItems(course.id) // 5 diskusi + 8 absen + 3 tugas
    ...
    expect(diskusi).toBe(5)
    expect(absen).toBe(8)
    expect(tugas).toBe(3)

    expect(
      items
        .filter(i => i.jenis === "DISKUSI")
        .map(i => i.sesi)
        .sort((a, b) => a - b)
    ).toEqual([1, 2, 4, 6, 8])
    expect(
      items
        .filter(i => i.jenis === "TUGAS")
        .map(i => i.sesi)
        .sort((a, b) => a - b)
    ).toEqual([3, 5, 7])
    expect(
      items
        .filter(i => i.jenis === "ABSEN")
        .map(i => i.sesi)
        .sort((a, b) => a - b)
...
describe("PATCH /api/tuton-items/:itemId", () => {
  ...
  it("should update status to SELESAI and set selesaiAt (OWNER)", async () => {
    ...
    const item = await TutonTest.findItem(course.id, JenisTugas.DISKUSI, 8)
  ...
  it("should allow USER to update nilai on DISKUSI", async () => {
    ...
    const item = await TutonTest.findItem(course.id, JenisTugas.DISKUSI, 4)
    expect(item).toBeTruthy()
...
describe("PATCH /api/tuton-items/:itemId/nilai", () => {
  ...
  it("should allow clearing nilai (set to null)", async () => {
    ...
    const diskusi = await TutonTest.findItem(course.id, JenisTugas.DISKUSI, 4)
