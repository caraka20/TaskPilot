...
import { StatusTugas, JenisTugas } from "../src/generated/prisma";
...
describe("GET /api/tuton/subjects", () => {
  ...
  it("should aggregate subjects (conflict & non-conflict) for OWNER", async () => {
    const c1 = await CustomerTest.create(); // e.g. A
    const c2 = await CustomerTest.create(); // e.g. B

    // Konflik: dua course dengan matkul sama
    await TutonTest.createCourse(c1.id, "Akuntansi Dasar");
    await TutonTest.createCourse(c2.id, "Akuntansi Dasar");

    // Non-konflik: satu saja
    await TutonTest.createCourse(c1.id, "Statistika");
    ...
    expect(res.status).toBe(200);
    const byMatkul = Object.fromEntries(
      (res.body.data as Array<{ matkul: string; totalCourses: number; isConflict: boolean }>)
        .map((x) => [x.matkul, x])
    );
  ...
  it("should support q filter", async () => {
    ...
    await TutonTest.createCourse(c.id, "Akuntansi Dasar");
    await TutonTest.createCourse(c.id, "Statistika");
    ...
    expect(res.status).toBe(200);
    const list = res.body.data as Array<{ matkul: string }>;
    expect(list.every((x) => x.matkul.includes("Akuntansi"))).toBe(true);
  });

  it("should allow USER to access", async () => {
    const c = await CustomerTest.create();
    await TutonTest.createCourse(c.id, "Manajemen Keuangan");
  ...
  it("should return 401 when no token", async () => {
    const res = await supertest(app).get("/api/tuton/subjects");
...
describe("GET /api/tuton/scan", () => {
  ...
  it("should list BELUM for ABSEN sesi=1 on specific subject (owner)", async () => {
    ...
    const A = await CustomerTest.create();
    const B = await CustomerTest.create();

    const courseA = await TutonTest.createCourse(A.id, "Akuntansi Dasar");
    const courseB = await TutonTest.createCourse(B.id, "Akuntansi Dasar");

    await TutonTest.createItems(courseA.id);
    await TutonTest.createItems(courseB.id);

    // Mark ABSEN sesi 1 as SELESAI for courseA only
    await prismaClient.tutonItem.updateMany({
      where: { courseId: courseA.id, jenis: JenisTugas.ABSEN, sesi: 1 },
      data: { status: StatusTugas.SELESAI, selesaiAt: new Date() },
    });
  ...
  it("should list two BELUM for DISKUSI sesi=1 (both courses)", async () => {
    ...
    const cA = await TutonTest.createCourse(A.id, "Akuntansi Dasar");
    const cB = await TutonTest.createCourse(B.id, "Akuntansi Dasar");

    await TutonTest.createItems(cA.id);
    await TutonTest.createItems(cB.id);
    ...
    expect((res.body.data.rows as any[]).length).toBe(2);
  });
  ...
  it("should paginate results (pageSize=1)", async () => {
    ...
    const cA = await TutonTest.createCourse(A.id, "Tugas Project");
    const cB = await TutonTest.createCourse(B.id, "Tugas Project");
    await TutonTest.createItems(cA.id);
    await TutonTest.createItems(cB.id);
    ...
    expect((page1.body.data.rows as any[]).length).toBe(1);
    ...
    expect((page2.body.data.rows as any[]).length).toBe(1);
  });

  it("should allow USER to access scan", async () => {
    const C = await CustomerTest.create();
    const c = await TutonTest.createCourse(C.id, "Basis Data");
    await TutonTest.createItems(c.id);
  ...
  it("should return 401 when no token", async () => {
    const res = await supertest(app).get("/api/tuton/scan").query({ jenis: "ABSEN", sesi: 1 });
    expect(res.status).toBe(401);
    expect(res.body.status).toBe("error");
  });
});
