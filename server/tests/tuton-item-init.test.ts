...
describe("POST /api/tuton-courses/:courseId/items/init", () => {
  ...
  it("should init 16 default items for course (OWNER)", async () => {
    ...
    expect(data).toHaveProperty("totalItems", 16)
    ...
    expect(course?.totalItems).toBe(16)
  ...
  it("should recreate when overwrite=true", async () => {
    ...
    expect(res.body.data).toHaveProperty("totalItems", 16)
...
describe("POST /api/tuton-courses/:courseId/items/bulk-status", () => {
  ...
  beforeEach(async () => {
    ...
    courseId = course.id
    await TutonTest.createItems(courseId) // 16 items
  })
  ...
  it("should allow USER to bulk update", async () => {
    ...
    const i2 = await TutonTest.findItem(courseId, JenisTugas.DISKUSI, 6)
  ...
  it("should be fine updating mix BELUM/SELESAI (idempotent behavior)", async () => {
    ...
    const i2 = await TutonTest.findItem(courseId, JenisTugas.DISKUSI, 8)
...
describe("POST /api/tuton-courses/:courseId/items/bulk-nilai", () => {
  ...
  it("should update nilai for multiple items (OWNER)", async () => {
    ...
    const disk8 = await TutonTest.findItem(course.id, JenisTugas.DISKUSI, 8)
    ...
    const d = await prismaClient.tutonItem.findUnique({ where: { id: disk8.id } })
  ...
  it("should return 400 if any item does not belong to the course", async () => {
    ...
    const i1 = await TutonTest.findItem(c1.id, JenisTugas.DISKUSI, 2)
  ...
  it("should return 401 when no token provided", async () => {
    ...
    await TutonTest.createItems(course.id)
    const d4 = await TutonTest.findItem(course.id, JenisTugas.DISKUSI, 4)

    const res = await supertest(app)
      .post(`/api/tuton-courses/${course.id}/items/bulk-nilai`)
      .send({ items: [{ itemId: d4.id, nilai: 65 }] })
...
describe("GET /api/tuton-courses/:courseId/summary", () => {
  ...
  it("should return correct summary for OWNER", async () => {
    const cust = await CustomerTest.create()
    const course = await TutonTest.createCourse(cust.id, "Manajemen")
    await TutonTest.createItems(course.id) // 16 items (5 diskusi, 8 absen, 3 tugas)
    ...
    await prismaClient.$transaction([
      ...
      prismaClient.tutonItem.update({ where: { id: d4.id }, data: { status: StatusTugas.SELESAI } }),
    ...
    // total items & completed (harus 16 & 6)
    const totals = data.totals || data.total || data.summary || data
    expect(totals.totalItems ?? data.totalItems).toBe(16)
    const completed = totals.completedItems ?? data.completedItems
    expect(completed).toBe(6)

    // persen progres antara 0..100 (≈ 6/16*100)
    ...
    if (data.jenis) {
      expect(data.jenis.DISKUSI.total).toBe(5)
