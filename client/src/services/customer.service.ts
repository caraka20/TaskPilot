...
function filenameFromContentDisposition(value?: string) {
  if (!value) return "taskpilot-customer.xlsx";

  const utf8 = value.match(/filename\*=UTF-8''([^;]+)/i);
  if (utf8?.[1]) return decodeURIComponent(utf8[1].trim());

  const regular = value.match(/filename="?([^";]+)"?/i);
  return regular?.[1]?.trim() || "taskpilot-customer.xlsx";
}

/** Download seluruh customer beserta detailnya sebagai workbook multi-sheet. */
export async function exportCustomersExcel() {
  const response = await httpClient.get<Blob>(`${base}/export/excel`, {
    responseType: "blob",
  });
