...
export function useIdleAutoPause({
  ...
  const touch = (at = Date.now()) => {
    ...
    try { localStorage.setItem("wk:lastActivity", String(at)); } catch { /* storage dapat diblokir browser */ }
    try { bcRef.current?.postMessage({ t: "act", at }); } catch { /* channel mungkin sudah ditutup */ }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try { bc.removeEventListener("message", onBC); bc.close(); } catch { /* cleanup best-effort */ }
      bcRef.current = null;
    };
