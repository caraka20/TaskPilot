...
export default function CustomerTable({
  ...
  const classNames = {
    table: canManage ? "min-w-[1080px]" : "min-w-[760px]",
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-3 md:hidden">
        {loading ? (
          <div className="rounded-2xl border border-default-200 bg-content2 px-4 py-10 text-center text-sm text-foreground-500">
            Memuat customer…
          </div>
        ) : sorted.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-default-300 bg-content2 px-4 py-10 text-center text-sm text-foreground-500">
          ...
          sorted.map((row, idx) => (
            <article
              key={row.id}
              className="rounded-3xl border border-default-200 bg-content1 p-4 shadow-sm"
            >
              <div className="flex items-start gap-3">
                <span className={`${numberBadge} mt-0.5 shrink-0`}>{nomor(idx)}</span>
              ...
              {canManage && (
                <div className="mt-4 grid grid-cols-1 gap-3 rounded-2xl bg-default-50 p-3 min-[380px]:grid-cols-2 dark:bg-default-100/50">
                  <div>
              ...
              <div className="mt-4 flex gap-2">
                <Button
                  ...
                  color="primary"
                  variant="flat"
                  className="min-h-11 flex-1 rounded-2xl font-semibold"
                >
                  Lihat Detail
                </Button>
                {canManage && (
