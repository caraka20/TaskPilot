  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-3 md:hidden">
        {loading ? (
          <div className="rounded-2xl border border-default-200 px-4 py-8 text-center text-sm text-foreground-500">
            Memuat pembayaran…
          </div>
        ) : rows.length === 0 ? (
          ...
          rows.map((row) => (
            <article key={row.id} className="rounded-2xl border border-default-200 bg-content2 p-4">
              <div className="flex flex-col gap-2 min-[360px]:flex-row min-[360px]:items-start min-[360px]:justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-foreground-400">Tanggal</p>
                  <p className="mt-1 text-sm font-semibold">{formatTanggalText(row.tanggalBayar)}</p>
                </div>
                <span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold tabular-nums text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-400/20">
                  {fmtRp(row.amount)}
                </span>
              </div>
              {row.catatan && (
                <div className="mt-3 border-t border-default-200 pt-3">
                  <p className="text-xs text-foreground-400">Catatan</p>
                  <p className="mt-1 text-sm text-foreground-700">{row.catatan}</p>
                </div>
              )}
            </article>
          ))
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-default-200 md:block">
      <Table
