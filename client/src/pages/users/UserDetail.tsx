  ...
  return (
    <div className="space-y-6 pb-4">
      <PageHeader
        ...
        title={data?.namaLengkap || "Profil User"}
        description={data ? `Kelola aktivitas kerja, konfigurasi, dan payroll @${data.username} dari satu halaman.` : "Detail operasional dan payroll pengguna."}
        icon={<UserRound className="h-5 w-5" />}
        backTo="/users"
      />

      {loading ? (
        <StatePanel kind="loading" title="Memuat profil user" description="Menyiapkan detail aktivitas dan payroll…" />
      ) : err ? (
        <StatePanel
          kind="error"
        ...
        <>
          ...
          {role === "OWNER" && (
            <OwnerGajiTable
              username={data.username}
              title={`Pembayaran ${data.namaLengkap}`}
              description="Catat dan kelola pembayaran gaji langsung dari profil user ini."
