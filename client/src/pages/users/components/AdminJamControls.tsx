  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-2">
        ...
        <Chip color={color} variant="flat" radius="lg" className="px-3 py-2 text-base font-medium">{label}</Chip>
      </div>
...
interface Props {
  /** username target (halaman owner) */
  targetUsername: string;
  /** status ringkas (“AKTIF” | “JEDA” | “TIDAK_AKTIF”) */
  status: AdminJamStatus;

  /** ID segmen yang sedang terbuka (jamSelesai=null) — untuk pause & end */
  activeSessionId?: number | null;

  /** ID baris jeda yang akan di-resume (bisa beda dari activeSessionId) */
  resumeTargetId?: number | null;

  /** akumulasi detik dari segmen SELESAI (tanpa delta segmen aktif sekarang) */
  detikBerjalan: number;
  /** ISO saat segmen aktif dimulai (jika status=AKTIF) */
  startedAt?: string | null;
  /** ISO waktu server (opsional) */
...
export default function AdminJamControls({
  ...
  const doStart = async () => {
    ...
    setLoading("start");
    try {
      await startJamKerjaForUser(api, targetUsername); // ← owner start utk user lain
      ok("Mulai kerja!");
      onChanged();
    } catch (e: any) {
