...
const DISKUSI_SESI = new Set([1, 2, 4, 6, 8]);
...
export default function PublicSessionTable({
  ...
  return (
    <div className="rounded-2xl border border-default-200 overflow-hidden bg-content1">
      ...
      <div className="border-b border-default-200 bg-content2/60 px-4 py-2 text-xs text-foreground-500 md:hidden">
        Geser ke samping untuk melihat seluruh sesi.
      </div>

      {/* Table head */}
      <div
        className="public-matrix-scroll max-w-full overflow-x-auto overscroll-x-contain touch-pan-x"
        role="region"
        aria-label="Laporan Tuton yang dapat digeser ke samping"
        tabIndex={0}
      >
      <div className="grid min-w-[1040px]"
        ...
        {courses.map((c) => (
          <React.Fragment key={c.courseId}>
            ...
            {SESSIONS.map((s) => {
              ...
              const t = TUGAS_SESI.has(s) ? findItem(c, "TUGAS", s) : undefined;
